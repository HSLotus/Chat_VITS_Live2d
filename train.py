...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
    ...
    if rank == 0:
        ...
        sql_L1 = str(format(loss_disc, '.2f'))
        sql_L2 = str(format(loss_gen, '.2f'))
        sql_L3 = str(format(loss_fm, '.2f'))
        sql_L4 = str(format(loss_mel, '.2f'))
        sql_L5 = str(format(loss_kl, '.2f'))
        sql_Lr = str(format(optim_g.param_groups[0]['lr'], '.10f'))
        sql_step = str(format(global_step, '.2f'))
        sql_reference_loss = str(format(loss_disc+loss_gen+loss_fm+loss_mel+loss_kl, '.2f'))

        cursor.execute(
            "insert into train (L1,L2,L3,L4,L5,Lr,reference_loss,step,epoch,cost) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
            [(sql_L1),(sql_L2),(sql_L3),(sql_L4),(sql_L5),(sql_Lr),(sql_reference_loss),(sql_step),(str(epoch)),(str(durtaion))])
